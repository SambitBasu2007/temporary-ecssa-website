...
// Description labels reuse the title font at roughly two-thirds size.
function shrinkFont(font: string): string {
  return font.replace(/(\d+)px/i, (_match, px: string) => `${Math.max(10, Math.round(Number(px) * 0.66))}px`);
}
...
interface TitleProps {
  ...
  textColor?: string;
  font?: string;
  /** Label height as a fraction of the plane's local height (default 0.15). */
  scaleFactor?: number;
  /** Local y where the label's top edge sits; defaults to just under the plane. */
  topY?: number;
}
...
class Title {
  ...
  constructor({
    ...
    this.topY = topY ?? -plane.scale.y * 0.5 - 0.05;
    this.createMesh();
  }
  ...
  createMesh() {
    ...
    const textHeightScaled = this.plane.scale.y * this.scaleFactor;
    const textWidthScaled = textHeightScaled * aspect;
    this.mesh.scale.set(textWidthScaled, textHeightScaled, 1);
    this.mesh.position.y = this.topY - textHeightScaled * 0.5;
...
class Media {
  ...
  createTitle() {
    // plane.scale.y is still 1 here, so these offsets are in the plane's local
    // units and inherit the plane's onResize scaling automatically.
    const topY = -this.plane.scale.y * 0.5 - 0.05;
    ...
    if (this.description) {
      this.descriptionLabel = new Title({
        gl: this.gl,
        plane: this.plane,
        renderer: this.renderer,
        text: this.description,
        textColor: this.descriptionColor,
        font: shrinkFont(this.font ?? DEFAULT_FONT),
        scaleFactor: 0.078,
        topY: topY - 0.15 - 0.015
      });
    }
  }
...
class App {
  ...
  onResize() {
    ...
    // Lift the whole scene so the caption block under each plane (title +
    // description, ~0.315 of the plane height) stays inside the viewport,
    // compensating the extra dip the bend gives the edge planes. The world
    // sizes here are constant (the frustum does not depend on stage pixels),
    // so this resolves to a fixed shift. Captions win over the photo's top
    // edge: if the stage is too short for both, the photos crop slightly at
    // the top instead of the labels getting clipped at the bottom.
    if (this.medias && this.medias[0]) {
      const p = this.medias[0].plane.scale.y;
      const half = this.viewport.height / 2;
      const minLift = p * 0.5 + p * 0.36 - half + Math.abs(this.bend) * 0.6 + 0.15;
      this.scene.position.y = Math.min(minLift, p * 0.5 - 0.5);
    }
  }
